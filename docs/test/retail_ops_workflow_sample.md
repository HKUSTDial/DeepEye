...
## Prompt 1: Dashboard
```text
我刚上传了一份 2025Q4 城市经营目标与营销预算表，也连接了门店销售数据库。请基于这些数据整理一份城市经营分析数据集，计算每个城市的总营收、目标达成率、预算 ROI、会员订单渗透率，并生成一个 dashboard，让我能看到城市排名、目标完成情况和预算效率。
```

## Prompt 2: Report
```text
我刚上传了一份 2025Q4 城市经营目标与营销预算表，也连接了门店销售数据库。请先把数据库里的城市销售结果和 CSV 中的城市目标、预算信息合并，计算目标达成率和预算 ROI，然后生成一份简洁的经营分析报告，指出哪些城市营收高、哪些城市低于目标，以及后续优先动作。
```

## Prompt 3: Video
```text
我刚上传了一份 2025Q4 城市经营目标与营销预算表，也连接了门店销售数据库。请基于这些数据做一个中文总结视频，说明哪个城市贡献的总营收最高、前几名城市的排序、各城市的目标完成情况，以及应该优先关注的城市。
```
