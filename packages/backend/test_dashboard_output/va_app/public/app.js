...
const App = {
    ...
    init: async function() {
        ...
        try {
            ...
            if(root) root.innerHTML = `<div class="p-8 text-red-500">Failed to load dashboard: ${e.message}</div>`;
        }
    },
      ...
      filters.forEach(f => {
          ...
          // 3. Range / Slider (双滑块)
              ...
              const debouncedSend = debounce(() => {
                  ...
                  this.sendFilter(field, [vMin, vMax]);
              }, 300);
          ...
          // 4. Date Range (日期范围)
                  ...
                  const handleDate = () => {
                      ...
                      const v2 = dMax.value;
                      if (v1 && v2) {
                          this.sendFilter(field, [v1, v2]);
                      } else {
                          this.sendFilter(field, null);
                      }
                  };
        ...
        if(this.socket && this.socket.readyState === WebSocket.OPEN) {
            ...
            if (val !== null) {
                ...
                if (Array.isArray(val)) {
                    // 如果是数字对或日期对，通常是 between，否则是 in
                    if (val.length === 2 && (typeof val[0] === 'number' || !isNaN(Date.parse(val[0])))) {
                        op = 'between';
                    } else {
                        op = 'in'; // MultiSelect
                    }
                ...
                // 发送空值以清除过滤器
                filterObj[field] = { operator: 'equals', value: null };
            }
