    ...
    renderFilters: function(blocks) {
      ...
      filters.forEach(f => {
          ...
          // 4. Date Range (日期范围)
            ...
            // 使用 type="text" 并添加 placeholder 以支持英文格式显示
            dateWrap.innerHTML = `
            <div class="relative group">
                ...
                <div class="relative">
                    ...
                    <input type="text" id="date-min-${field}" 
                           placeholder="YYYY-MM-DD" 
                           autocomplete="off"
                           class="date-picker-input w-full pl-9 pr-3 py-1.5 text-sm border border-gray-200 rounded-lg bg-white focus:ring-2 focus:ring-[#E18182] focus:outline-none transition-shadow text-gray-700">
                </div>
            </div>
            <div class="relative group">
                ...
                <div class="relative">
                    ...
                    <input type="text" id="date-max-${field}" 
                           placeholder="YYYY-MM-DD" 
                           autocomplete="off"
                           class="date-picker-input w-full pl-9 pr-3 py-1.5 text-sm border border-gray-200 rounded-lg bg-white focus:ring-2 focus:ring-[#E18182] focus:outline-none transition-shadow text-gray-700">
                </div>
            ...
            // 使用 requestAnimationFrame 确保在 DOM 渲染后初始化 Flatpickr
            requestAnimationFrame(() => {
                if (typeof flatpickr === 'undefined') {
                    console.warn('Flatpickr not found, falling back to native date input behavior.');
                    return;
                }
                ...
                const commonCfg = {
                    dateFormat: "Y-m-d",
                    allowInput: true,
                    disableMobile: true, // 强制使用 Flatpickr 而非手机原生控件
                    locale: {
                        firstDayOfWeek: 1,
                        weekdays: {
                ...
                flatpickr(dMin, { ...commonCfg, defaultDate: minD });
                flatpickr(dMax, { ...commonCfg, defaultDate: maxD });
