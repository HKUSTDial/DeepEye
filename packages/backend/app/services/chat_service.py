...
def start_agent_workflow(session_id: str, message: str, datasource_id: str | None = None) -> str:
    """Start agent workflow and return task ID."""
    task = run_agent_workflow.delay(
        AgentInput(session_id=session_id, user_input=message, datasource_id=datasource_id).model_dump()
    )
    return task.id
