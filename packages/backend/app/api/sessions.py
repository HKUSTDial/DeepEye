...
from app.repositories import EventRepository, SessionRepository
...
def delete_session(session_id: uuid.UUID, db: Session = Depends(get_db)):
    ...
    EventRepository(db).delete_stream(str(session_id))
    repo.delete(session_id)


@router.get("/{session_id}/history")
def get_session_history(session_id: str, db: Session = Depends(get_db)) -> dict[str, Any]:
    """Retrieve event history for session reconstruction."""
    return {"events": EventRepository(db).get_stream(session_id)}
