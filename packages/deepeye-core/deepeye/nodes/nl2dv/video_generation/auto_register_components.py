...
def copy_components_to_project(components: List[Dict[str, str]], target_dir: str):
    """复制组件到项目目录"""
    os.makedirs(target_dir, exist_ok=True)
    
    print(f"\n📂 复制组件到项目...")
    for comp in components:
        target_path = os.path.join(target_dir, comp['filename'])
...
def update_root_tsx(components: List[Dict[str, str]], root_tsx_path: str, config_path: str):
    ...
    for comp in components:
        ...
        composition_id = comp['component_name']  # Composition ID（无下划线）
...
def main():
    ...
    # 配置路径
    if args.animated:
        output_dir = "infographic_generation/output/claude_tsx_animated"
        print("🚀 自动注册**动画**组件到 Remotion 项目...")
    else:
        output_dir = "infographic_generation/output/claude_tsx_components"
    ...
    # 2. 复制到项目
    copy_components_to_project(components, target_dir)
