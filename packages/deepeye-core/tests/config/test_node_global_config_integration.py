...
from deepeye.nodes.datasource import CSVDataSourceNode, FileDataSourceNode, MemoryDataSourceNode
...
class TestNodeGlobalConfigIntegration:
    ...
    def test_node_uses_global_config(self, tmp_path):
        ...
        node = CSVDataSourceNode(node_id="test")
    ...
    def test_multiple_nodes_with_different_configs(self, tmp_path):
        ...
        # 创建不同类型的节点
        csv_node = CSVDataSourceNode(node_id="csv")
        file_node = FileDataSourceNode(node_id="file")
        
        # 验证配置正确
        assert csv_node.config.file_path == str(csv_file)
    ...
    def test_global_config_isolation_between_instances(self, tmp_path):
        ...
        # 创建第一个节点
        node1 = CSVDataSourceNode(node_id="node1")
        
        # 修改节点1的配置（运行时修改）
        node1.config.encoding = "gbk"
        
        # 创建第二个节点
        node2 = CSVDataSourceNode(node_id="node2")
        
        # 节点2应该不受节点1的影响
        assert node2.config.encoding == "utf-8"  # 默认值
        
        # 全局配置也不应该被影响
        global_cfg = config.get_node_config("CSVDataSource")
...
class TestGlobalConfigWithAgentRegistration:
    ...
    def test_register_node_without_required_config_fails(self):
        ...
        # 注册节点类（不会失败，因为只是注册类）
        registry.register(CSVDataSourceNode)
        
        # 创建节点实例时会失败（没有file_path）
        with pytest.raises(Exception):  # 会抛出配置错误
            registry.create_node("CSVDataSource")
    ...
    def test_register_node_with_global_config_succeeds(self, tmp_path):
        ...
        registry = NodeRegistry()
        # 清除可能存在的注册
        if registry.is_registered("CSVDataSource"):
            registry.unregister("CSVDataSource")
        
        registry.register(CSVDataSourceNode)
        node = registry.create_node("CSVDataSource")
...
class TestGlobalConfigErrorHandling:
    ...
    def test_empty_global_config_with_required_field(self):
        """测试空全局配置但节点有必需字段"""
        # 不设置全局配置
        # CSVDataSource 需要 file_path
        
        # 创建节点应该失败（缺少必需字段）
        with pytest.raises(Exception):
            CSVDataSourceNode(node_id="test")
...
class TestGlobalConfigDocumentation:
    ...
    def test_basic_usage_example(self, tmp_path):
        ...
        node1 = CSVDataSourceNode(node_id="sales")
        assert node1.config.file_path == str(csv_file)
        
        # 显式 config 会覆盖全局配置
        node2 = CSVDataSourceNode(
            node_id="custom",
            config={"file_path": str(csv_file), "encoding": "gbk"}
        )
        assert node2.config.encoding == "gbk"
        
        # 部分覆盖
        node3 = CSVDataSourceNode(
            node_id="partial",
