  ...
  const narrations = [
    {
      ...
      "time_start": 1.193,
      "time_end": 3.289,
      "audio_file": "tmp/video_config_audio/analysis_monthly_sales_trend_narr0.wav"
    }
  ];
  ...
  useEffect(() => {
    ...
    if (entranceAnim) {
      ...
      if (frame >= animEnd) {
        ...
        const animDuration = entranceAnim.duration;
        
        // Line drawing animation
        const lineProgress = Math.min(totalTime / (animDuration * 0.6), 1);
        const lineEased = d3.easeCubicOut(lineProgress);
        
        const linePath = g.select('.line');
        const totalLength = linePath.node()?.getTotalLength() || 0;
        
        linePath
          .style('opacity', lineEased)
          .attr('stroke-dashoffset', totalLength * (1 - lineEased));
        
        // Grid lines fade in
        const gridStart = 0.2;
        if (totalTime >= gridStart) {
          const gridProgress = Math.min((totalTime - gridStart) / 0.4, 1);
        ...
        g.selectAll('.dot').each(function(d: any, i: number) {
          ...
          const delayPerDot = 0.15;
          const dotStart = 0.4 + i * delayPerDot;
          const dotDuration = 0.3;
          
          if (totalTime >= dotStart && totalTime <= dotStart + dotDuration) {
            const dotProgress = (totalTime - dotStart) / dotDuration;
            const eased = d3.easeBackOut(dotProgress);
            dot.style('opacity', eased);
          } else if (totalTime > dotStart + dotDuration) {
            dot.style('opacity', 1);
          }
        });
        ...
        g.selectAll('.value-label').each(function(d: any, i: number) {
          ...
          const labelStart = 0.6 + i * delayPerLabel;
    ...
    if (!hasActiveEmphasis && entranceAnim && frame >= (entranceAnim.time_start - sceneStartOffset + entranceAnim.duration) * fps) {
      g.selectAll('.dot').attr('stroke', backgroundColor).style('opacity', 1).style('filter', 'url(#shadow)');
      g.selectAll('.value-label').style('opacity', 1);
      g.selectAll('.grid-y').style('opacity', 0.3);
      g.selectAll('.x-axis-label, .y-axis-label').style('opacity', 1);
    }
